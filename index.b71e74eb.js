...
if (!buttonElement) throw new Error("Cannot find button element");
...
function run(input) {
    ...
    // Validate input
    const sampledCodes = input.replace(/[^\d\s]/g, "") // Remove non-digits, non-spaces
    .replace(/\s+/g, " ") // Remove redundant spaces
    .trim().split(" ");
    console.log(`sampled codes = ${sampledCodes.join(" ")}`);
    if (sampledCodes.length == 0) throw new Error("No combinations provided");
    ...
    const possibleCodes = [
        ...Array(Math.pow(10, codeLength)).keys()
    ].map((c)=>c.toString().padStart(codeLength, "0")).filter((c)=>!sampledCodes.includes(c));
    ...
    if (!tableElement) throw new Error("Cannot find table element");
    tableElement.innerHTML = "";
    const createRow = (col1, col2, header = false)=>{
...
},{}]},["3LmCz","h7u1C"], "h7u1C", "parcelRequire94c2")
